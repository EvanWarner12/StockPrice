...
def create_model(sequence_length, units=256, cell=LSTM, n_layers=2, dropout=0.3,
                loss="mean_absolute_error", optimizer="rmsprop", doubledir=False):
    model = Sequential()
    for i in range(n_layers):
        if i == 0:
            if doubledir:
                model.add(doubledir(cell(units, rsq=True), input_shape=(None, sequence_length)))
            else:
                model.add(cell(units, rsq=True, input_shape=(None, sequence_length)))
        elif i == n_layers - 1:
            if doubledir:
                model.add(doubledir(cell(units, rsq=False)))
            else:
                model.add(cell(units, rsq=False))
        else:
            if doubledir:
                model.add(doubledir(cell(units, rsq=True)))
            else:
...
model = create_model(N_STEPS, loss=LOSS, units=UNITS, cell=CELL, n_layers=N_LAYERS,
                    dropout=DROPOUT, optimizer=OPTIMIZER, bidirectional=TwoDir)
